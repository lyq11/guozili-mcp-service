#!/usr/bin/env python3
"""离线 PCB 布局原型的 unittest 用例,覆盖两轮 review 里提出的具体场景:
修正后的网络评分公式、非正交角度下的区域刚体变换、带孔洞的板框、
锁定器件作为障碍物但自己永不移动、连接器允许出板框的量、以及
门禁在线长退化时强制拒绝。

运行方式:python -m unittest test_pcb_layout -v
"""
...
class NetScoringTests(unittest.TestCase):
    def test_ground_excluded_and_inverse_frequency(self):
        # C1 跟 U1 共享 VOUT(被 2 个器件用到:C1 + U1)和 GND(所有器件都用)。
        # GND 不应该贡献分数;VOUT 应该贡献 1/2。
    ...
    def test_common_rail_scores_lower_than_rare_net(self):
        # 一个只共享了"到处都在用"的电源轨的器件,得分应该低于一个共享了
        # "很少见"网络的器件——即便两者都只跟锚点共享恰好一个非地网络。
...
class RegionRigidTransformTests(unittest.TestCase):
    def test_member_position_rotation_and_bbox_move_together(self):
        ...
        # 成员相对锚点原本是 (50, 0);把这个偏移量旋转 +90 度(约定:
        # x'=x*cos-y*sin, y'=x*sin+y*cos)得到 (0, 50),所以成员应该落在
        # anchor_new + (0, 50) 的位置。
        ...
        # 成员的 bbox 必须同时反映位置变化和 90 度的重新定向(这里是个
        # 10x10 的正方形,旋转后还是 10x10,因为它对称;但要检查它是不是
        # 真的以新位置为中心,而不是还停留在旧位置上)。
        bbox = member.bbox
    ...
    def test_asymmetric_bbox_swaps_dimensions_on_90_degree_rotation(self):
        # 一个 20 宽 x 6 高的 bbox 绕自己的中心旋转 90 度,应该变成
        # 6 宽 x 20 高。
