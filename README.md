...
- 只缓存关联 `[main]` 原理图的 PCB Board，包括板框、层、封装焊盘、独立焊盘、网络、走线、过孔、铺铜和设计规则。
- 检查 PCB 未布线网络、游离直线导线、封装重叠、板外器件、原理图一致性和原生 DRC。
...
- 删除游离导线前会重新扫描；默认跳过含铺铜、圆弧/折线走线的网络以及锁定导线。
- PCB 写入支持显式创建铺铜、过孔和独立焊盘；焊盘的网络、层、编号、形状、孔和金属化属性均由调用方提供。
- PCB 建板与整理支持铜层数/物理叠层、板框、按网络批量走线策略，以及器件对齐、等间距和栅格吸附。
- 在编辑器中显示连接状态悬浮框。
...
需要诊断连接或工具调用时，可选择“果子狸MCP服务 → 开启/关闭 Debug 日志”。开关会持久化并触发一次重连；开启后，扩展在 EasyEDA 开发者控制台输出 RPC 调用、返回、耗时和异常，同时通知 MCP 网关为该编辑器窗口向 stderr 输出对应日志。认证令牌会脱敏，过大的参数或返回值会截断。也可仅在 MCP 进程设置 `EASYEDA_MCP_DEBUG=1`，强制记录所有窗口的 RPC 日志。
...
| `EASYEDA_MCP_DEBUG=1` | 强制向 stderr 输出全部 RPC 调用、返回、耗时和错误日志 |
