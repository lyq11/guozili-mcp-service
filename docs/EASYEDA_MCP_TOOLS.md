...
当前共有 **17 个 MCP 工具**。日常修改优先使用直接工具，`schematic_apply_operations` 仅作为尚未封装能力的底层入口。
...
| `schematic_list_pages` | 只读 | 列出当前原理图的全部图页 |
...
2. `schematic_place_components` 只放器件。
